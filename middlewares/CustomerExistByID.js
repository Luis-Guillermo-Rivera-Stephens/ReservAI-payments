...
const CustomerExistByID = async (req, res, next) => {
    ...
    try {
        ...
        SupportManager.sendSupportTicket(req, "Error 500 en el middleware de CustomerExistByID", error.message);
        return res.status(500).json({ error: 'Internal server error' });
    }
    const result = await CustomersManager.customerExistByID(account_id, db);
    if (result.error) {
        SupportManager.sendSupportTicket(req, "Error 500 en el middleware de CustomerExistByID", result.error);
